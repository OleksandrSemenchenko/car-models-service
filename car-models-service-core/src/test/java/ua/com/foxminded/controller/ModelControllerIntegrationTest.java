...
class ModelControllerIntegrationTest {
    ...
    void search_ShouldReturnStatus200_WhenParametersArePresent() throws Exception {
        mockMvc.perform(get("/models").param("model", MODEL_NAME)
    ...
    void getById_ShouldReturnStatus200() throws Exception {
        mockMvc.perform(get("/models/{id}", MODEL_ID))
               .andExpect(status().isOk())
    ...
    void create_ShouldReturnStatus201() throws Exception {
        ...
        mockMvc.perform(post("/manufacturers/{manufacturer}/models/{name}/{year}", 
                             MANUFACTURER_NAME, 
                             MODEL_NAME, 
                             NEW_MODEL_YEAR)
               ...
               .andExpect(header().string("Location", containsString("/models/")));
    }
